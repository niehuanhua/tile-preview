...
- 验收测试（需要 Node，直接跑即可）：
  ...
  三个文件里都有 `/*ENGINE-START*/…/*ENGINE-END*/` 两行标记，之间的纯函数会被测试用正则抽出来单独运行，
...
**三样东西要一起改**：链接里的 `?v=N`、那张卡片右上角的版本号徽章 `vN`、
以及页头的 `更新：` 时间。徽章和 `?v=N` 是同一个数字——手机上点开 `home.html`
一眼就能看见现在装的是第几版，不用去猜缓存有没有生效。只改一个等于白标。
